...
use Kdig\ArchaeologicalBundle\Form\MatrixFilterType;
...
class MatrixController extends Controller
{
    ...
    */
    protected function filter()
    {
        ...
        $filterForm = $this->createForm(new MatrixFilterType());
        $em = $this->getDoctrine()->getManager();
        $queryBuilder = $em->getRepository('KdigArchaeologicalBundle:Matrix')->createQueryBuilder('e');

        // Reset filter
        if ($request->get('filter_action') == 'reset') {
            $session->remove('MatrixControllerFilter');
        }
        ...
        if ($request->get('filter_action') == 'filter') {
            ...
            if ($filterForm->isValid()) {
                ...
                $session->set('MatrixControllerFilter', $filterData);
            }
        } else {
            // Get filter from session
            if ($session->has('MatrixControllerFilter')) {
                $filterData = $session->get('MatrixControllerFilter');
                $filterForm = $this->createForm(new MatrixFilterType(), $filterData);
                $this->get('lexik_form_filter.query_builder_updater')->addFilterConditions($filterForm, $queryBuilder);
            }
        }
    ...
    */
    protected function paginator($queryBuilder)
    {
        ...
        {
            return $me->generateUrl('matrix', array('page' => $page));
        };
